...
export default class Login {
  ...
  handleSubmitAdmin = e => {
    const user = {
      type: "Admin",
      email: e.target.querySelector(`input[data-testid="admin-email-input"]`).value, // bug here
      password: e.target.querySelector(`input[data-testid="admin-password-input"]`).value, // bug here
      status: "connected"
    }
    this.localStorage.setItem("user", JSON.stringify(user))
    const userExists = this.checkIfUserExists(user)
